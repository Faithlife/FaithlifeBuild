...
#pragma warning restore CA2025
					...
					Action<string>? packageSigner = null;
					var pushSuccess = false;

					if (packagePaths.Count != 0 && packageSettings?.SigningSettings is { } signingSettings)
						packageSigner = PackageSigningHelper.CreatePackageSigner(signingSettings);

					foreach (var packagePath in packagePaths)
					{
						if (packageSigner is not null)
						{
